...
label1 = Label(root, text="Search", font="Helvetica 20 bold",
               bg="#E1E8ED")  # get data from the user
...
def tweet():  # master code,here lies the main code for analysis
    ...
    try:
        ...
        neutral = 0
        for t in tweets:
            # clean_data()is defined after this function
        ...
        T = Text(root, height=9, width=50, bd=5,
                 font="Helvetica 15", bg="#F5F8FA")
        T.pack()
        T.bell()
        T.insert(
            END, "********************************************************************"+"\n")
        ...
        # graph() is defined after clean_data()
        ...
        '''
        msg.showerror(
            "Error", "OOPS!!!Twitter doesn't have any tweets regarding the entered topic")
...
def graph(positive, negative, neutral, topics):  # plotting Graph
    ...
    plt.bar(index, positive, bar_width, alpha=opacity,
            color='g', edgecolor='w', label='positive')

    plt.bar(index + bar_width, negative, bar_width, alpha=opacity,
            color='r', edgecolor='w', label='negative')

    plt.bar(index + bar_width + bar_width, neutral, bar_width,
            alpha=opacity, color='b', edgecolor='w', label='neutral')
    ...
    plt.title('Twitter Sentiment Analysis', fontweight='bold', color='white',
              fontsize='17', horizontalalignment='center', backgroundcolor='black')
    plt.legend()
...
submit = Button(root, text="Submit", command=tweet,
                font="Helvetica 16", bg="#E1E8ED", bd=5, relief="raised")
label1.pack()
E1.pack()
